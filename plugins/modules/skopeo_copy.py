"""`skopeo_copy` Ansible plugin inside the `local.skopeo` collection.
"""
...
RETURN = r"""
changed:
    description: Returns "false" if the copy attempt fails; returns "true" otherwise.
    type: bool
    returned: always
    sample: false
return_code:
...
def run_module():
    ...
    if module.params["src_username"] and module.params["src_password"]:
        skopeo_command_args.append(
            f"--src-creds={module.params['src_username']}:{module.params['src_password']}"
        )

    if module.params["dest_username"] and module.params["dest_password"]:
        skopeo_command_args.append(
            f"--dest-creds={module.params['dest_username']}:{module.params['dest_password']}"
        )

    skopeo_command_args.extend(
        [module.params["src_image"], module.params["dest_image"]]
    )
